...
const ClientList = () => {
  ...
  const fetchClients = async () => {
    try {
      setLoading(true);
      const response = await adminService.clients.getAll();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setDeleteLoading(id);
      await adminService.clients.delete(id);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 pb-4 border-b-2 border-[#E8DCC4]">
        <h1 className="text-2xl md:text-3xl font-bold bg-linear-to-r from-[#C4A962] to-[#D4AF37] bg-clip-text text-transparent uppercase tracking-tight">Manage Clients</h1>
        <button
      ...
      {clients.length === 0 ? (
        <div className="text-center py-12 bg-white shadow-lg border-2 border-[#E8DCC4]">
          <p className="text-[#6B6B6B] font-medium">No clients found. Create your first client!</p>
        </div>
      ) : (
          ...
          {clients.map((client) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-[#C4A962] font-medium mb-2">
                  {client.designation}
                </p>
