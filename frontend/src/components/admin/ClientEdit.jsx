...
const ClientEdit = () => {
  ...
  const fetchClient = async () => {
    try {
      setLoading(true);
      const response = await adminService.clients.getById(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await adminService.clients.update(id, data);
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
      <div className="mb-6 pb-4 border-b-2 border-[#E8DCC4]">
        ...
        >
          ...
          <span className="text-sm md:text-base">Back to Clients</span>
        </button>
        <h1 className="text-2xl md:text-3xl font-bold bg-linear-to-r from-[#C4A962] to-[#D4AF37] bg-clip-text text-transparent uppercase tracking-tight">Edit Client</h1>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          ...
          >
            {saving ? (
              ...
              <span className="text-sm md:text-base">Update Client</span>
            )}
          </button>
          <button
            type="button"
